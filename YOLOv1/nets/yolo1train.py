...
class Train(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        total_loss = (coor_loss + obj_confi_loss + noobj_confi_loss + class_loss)/n_batch
        # 此处可以写代码验证一下loss的大致计算是否正确，这个要验证起来比较麻烦，比较简洁的办法是，将输入的pred置为全1矩阵，再进行误差检查，会直观很多。
        losses = [coor_loss,obj_confi_loss,noobj_confi_loss,class_loss,total_loss]
        return LossTuple(*losses)

    def train_step(self,pred, labels):
        self.optimizer.zero_grad()
        losses = self.forward(pred, labels)
